...
The single-factor specification is deliberately interpretable. Under a constant-volatility GBM, the diffusion coefficient does not have to change mechanically when moving from $\mathbb P$ to $\mathbb Q$. Using a historical estimate of $\sigma$ in Equation~\eqref{eq:futures_q} is therefore coherent under the maintained model. A market-implied effective volatility can nevertheless differ because observed option prices reflect risk premia and because the one-factor constant-volatility benchmark may absorb omitted stochastic volatility, jumps, smile effects, or richer futures-curve dynamics.
