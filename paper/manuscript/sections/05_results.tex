...
The stronger test predicts each target date using only implied-volatility observations from earlier dates. Table~\ref{tab:forward_q} reports the resulting errors over 345 holdout option-date observations. No same-day option settlement enters the fitted smile.

\begin{table*}[t]
\centering
\caption{Strict forward-in-time APO volatility validation. The historical-volatility baseline is the posterior-integrated price on the same holdout observations.}\label{tab:forward_q}
\begin{tabular}{llrrr}
\toprule
Sample & Volatility specification & $N$ & MAE & RMSE\\
\midrule
All holdouts & Historical posterior-integrated & 345 & 0.3583 & 0.4816\\
All holdouts & Expanding prior-date smile & 345 & 0.1297 & 0.2512\\
All holdouts & Previous-day smile & 345 & 0.1312 & 0.2312\\
\addlinespace
Positive-volume & Historical posterior-integrated & 8 & 0.1297 & 0.1639\\
Positive-volume & Expanding prior-date smile & 8 & 0.0959 & 0.1339\\
Positive-volume & Previous-day smile & 8 & 0.1002 & 0.1332\\
\bottomrule
\end{tabular}
\end{table*}

In the full holdout panel, the expanding smile reduces MAE by approximately 64\% and RMSE by approximately 48\%; the previous-day smile reduces MAE by approximately 63\% and RMSE by approximately 52\%. Mean error also moves much closer to zero, from $-0.3571$ under the historical-volatility baseline to $-0.0685$ and $-0.0591$, respectively.

The forward result changes the interpretation of the empirical exercise. The baseline negative pricing bias is not primarily a consequence of failing to integrate the historical volatility posterior. Information contained in the option-implied volatility surface, even when restricted to earlier dates, explains a much larger share of the subsequent pricing discrepancy. The empirical hierarchy is therefore
\begin{equation}
 \text{posterior-integration refinement}
 \ll
 \text{historical-versus-APO-implied volatility specification}
\end{equation}
for the observed September--October WTI panels.

This hierarchy does not eliminate a role for Bayesian parameter uncertainty. The mechanism map shows exactly where it can become first-order. Rather, the market experiment identifies the dominant margin in the data presently available: improving the volatility state used by the pricing model matters substantially more than integrating a relatively concentrated historical posterior through a smooth local pricing map.
