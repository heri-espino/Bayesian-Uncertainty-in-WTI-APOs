...
For each target APO date $t$, every LO observation used to fit the external surface satisfies $s<t$. The \emph{previous-day} surface uses only the latest completed LO reference date; the \emph{expanding} surface uses all earlier reference dates with a five-calendar-day exponential half-life. The fitted surface is evaluated separately for CLX6 and CLZ6 at each APO strike and combined with the fixing-count variance weights in Equation~\eqref{eq:lo_effective_sigma}. Same-day APO prices are never used to estimate the external state.
