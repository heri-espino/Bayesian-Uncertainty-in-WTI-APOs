...
def test_expected_average_uses_realized_and_forward_fixings():
    assert expected_average_level(np.array([90.0]), np.array([100.0, 110.0])) == 100.0


def test_call_decreases_and_put_increases_with_strike():
    kwargs = dict(
    ...
    call_low = wti_average_price_option_mc(strike=95.0, option_type="call", **kwargs).price
    call_high = wti_average_price_option_mc(strike=105.0, option_type="call", **kwargs).price
    put_low = wti_average_price_option_mc(strike=95.0, option_type="put", **kwargs).price
    put_high = wti_average_price_option_mc(strike=105.0, option_type="put", **kwargs).price
