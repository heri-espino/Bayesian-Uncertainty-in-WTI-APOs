...
> **Research status:** publication-oriented refactor on `main`. The legacy notebook is preserved for traceability, while the current codebase contains a rewritten manuscript, physical-measure inference, risk-neutral pricing, reproducible synthetic validation, and checkpointed large-scale experiments.
...
- `requirements-publication.txt`: base research dependencies.
- `requirements-gpu.txt`: optional CUDA/CuPy environment.
...
For the NVIDIA workstation, first verify the installed driver:

```bash
nvidia-smi
```

Then, if CUDA 12.x is appropriate for that machine:
...
The visually confirmed machine has an **NVIDIA RTX 4500 Ada Generation**, approximately **64 GB RAM**, an **Intel UHD Graphics 770** iGPU, and an **NVMe SSD**. Exact CPU model and dedicated RTX VRAM were not visible in the supplied Task Manager capture; runtime manifests query the machine rather than guessing these values. See [`research/COMPUTE.md`](research/COMPUTE.md).
...
### 1. Precompute pricing grids with point-level checkpoints
...
### 2. Run/resume CPU-parallel posterior experiments
...
```bash
python -m experiments.large_scale_synthetic --preset extreme --backend cupy --workers 24
```
...
A disconnected session should therefore be handled by simply rerunning the same command. Completed units remain on disk.
...
- `research/COMPUTE.md`: workstation specifications, compute policy, presets, checkpointing, and recommended execution sequence.
